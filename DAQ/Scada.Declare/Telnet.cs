...
namespace Scada.Declare
{
	public class Telnet
	{
		...
		static byte[] NewLine = new byte[]{(byte)'\r', (byte)'\n'};
		/// <summary>
		/// 流
		/// /// </summary>
		...
		//===================================================================
		public Telnet(string Address, int Port, int CommandTimeout = 500)
		{
			address = Address;
			port = Port;
		...
		public bool Connect()
		{
			try
			{
				...
				// AsyncCallback connectCallback = new AsyncCallback(OnConnect);
		...
		private void RespondToOptions()
		{
			try
			{
				...
				byte[] bytes = Encoding.ASCII.GetBytes(m_strResp);
		...
		void DispatchMessage(byte[] msg)
		{
			try
			{

				//发送char流,之后发送完毕后执行委托中的方法(此处为匿名委托)   
				IAsyncResult ar2 = s.BeginSend(msg, 0, msg.Length, SocketFlags.None, delegate(IAsyncResult ar)
				{
					//当执行完"发送数据" 这个动作后                 
